...
{* -----------------------------title------------------------------------ *}{...}
{p 0 16 2}
{bf:[W-1] wmtsum} {hline 2} output summary statistics to Stata interface, 
Word as well as LaTeX. The source code can be gained in {browse "https://github.com/Meiting-Wang/wmtsum":github}.
...
{p 4 4 2}
{bf:wmtsum}, based on esttab, can output summary statistics to  Stata interface, 
Word as well as LaTeX. User can use this command easily due to its simple syntax.
...
{p2col :{space 2}{opth p:age(strings:string)}}Set the extra package for the LaTeX. Don't need to care about the package of booktabs, array and dcolumn, because option {bf:alignment} will deal with it automatically{p_end}
{p2line}
{p2colreset}{...}
...
{p 4 4 2}Setup{p_end}
{p 8 8 2}. {stata sysuse nlsw88.dta, clear}{p_end}
...
{p 8 8 2}. {stata wmtsum wage age race hours}{p_end}

{p 4 4 2}Report summary statistics of specified statistics and specified numeric format{p_end}
{p 8 8 2}. {stata wmtsum wage age race hours, s(N sd(3) min(%9.2f) p99(%9.3fc))}{p_end}

{p 4 4 2}Add a custom title to the table{p_end}
{p 8 8 2}. {stata wmtsum wage age race hours, s(N sd(3) min(%9.2f) p99(%9.3fc)) ti(this is a title)}{p_end}

{p 4 4 2}Output the result to a .rtf file{p_end}
{p 8 8 2}. {stata wmtsum wage age race hours using Myfile.rtf, replace s(N sd(3) min(%9.2f) p99(%9.3fc)) ti(this is a title)}{p_end}

{p 4 4 2}Output the result to a .tex file{p_end}
{p 8 8 2}. {stata wmtsum wage age race hours using Myfile.tex, replace s(N sd(3) min(%9.2f) p99(%9.3fc)) ti(this is a title)}{p_end}

{p 4 4 2}Format table column in LaTeX to decimal point alignment{p_end}
{p 8 8 2}. {stata wmtsum wage age race hours using Myfile.tex, replace s(N sd(3) min(%9.2f) p99(%9.3fc)) ti(this is a title) a(dot)}{p_end}
